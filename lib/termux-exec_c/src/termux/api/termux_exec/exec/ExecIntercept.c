...
#include <termux/termux_core__c/v1/data/DataUtils.h>
#include <termux/termux_core__c/v1/file/FileUtils.h>
#include <termux/termux_core__c/v1/logger/Logger.h>
#include <termux/termux_core__c/v1/os/selinux/SeLinuxUtils.h>
#include <termux/termux_core__c/v1/shell/command/environment/AndroidShellEnvironment.h>
...
#include <termux/termux_exec__c/v1/TermuxExecLibraryConfig.h>
...
int execveIntercept(bool intercept, const char *executablePath, char *const argv[], char *const envp[]) {
    ...
    int result;
    if (!isTermuxExecExecveInterceptEnabled()) {
...
int execveInterceptInternal(const char *origExecutablePath, char *const argv[], char *const envp[]) {
    ...
    if (fd == -1) {
        errno = ENOENT;
        logStrerrorDebug(LOG_TAG, "Failed to open executable path '%s'", processedExecutablePath);
        return -1;
    }
    ...
    if (bytesRead < 0) {
        logStrerrorDebug(LOG_TAG, "Failed to read executable path '%s'", processedExecutablePath);
        return -1;
    }
    close(fd);
    ...
    if (inspectFileHeader(NULL, header, bytesRead, &info) != 0) {
        return -1;
    }
...
int inspectFileHeader(const char *termuxPrefixDir, char *header, size_t headerLength,
    struct FileHeaderInfo *info) {
    if (headerLength >= 20 && !memcmp(header, ELFMAG, SELFMAG)) {
        info->isElf = true;
    ...
    bool isRunningTests = libtermux_exec__c__getIsRunningTests();
    ...
    *newlineIndex = 0;
    ...
    if (whitespaceIndex != NULL) {
        ...
        *whitespaceIndex = 0;
        ...
        if (interpreterArg != newlineIndex) {
            size_t interpreterArgBufferLength = sizeof(info->interpreterArgBuffer);

            size_t interpreterArgLength = strlen(interpreterArg);
            if (interpreterArgBufferLength <= interpreterArgLength) {
                if (!isRunningTests) {
                    logErrorDebug(LOG_TAG, "The interpreter argument '%s' with length '%zu' is too long to fit in the buffer with length '%zu'",
                        interpreterArg, interpreterArgLength, interpreterArgBufferLength);
                }
                errno = ENAMETOOLONG;
                return -1;
            }
    ...
    size_t interpreterPathBufferLength = sizeof(info->interpreterPathBuffer);
    ...
    if (interpreterPath[0] == '/') {
        ...
        info->interpreterPath = termuxPrefixPath(LOG_TAG, termuxPrefixDir,
            interpreterPath, info->interpreterPathBuffer, interpreterPathBufferLength);
        ...
        if (interpreterPathBufferLength <= processedInterpreterPathLength) {
            if (!isRunningTests) {
                logErrorDebug(LOG_TAG, "The processed interpreter path '%s' with length '%zu' is too long to fit in the buffer with length '%zu'",
                    interpreterPath, processedInterpreterPathLength, interpreterPathBufferLength);
            }
            errno = ENAMETOOLONG;
            return -1;
        }
