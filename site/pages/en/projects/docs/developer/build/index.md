...
# For `libtermux-core_c` as build dependency.
...
To build `termux-core` package as well and use its local `libtermux-core_c` build as build dependency for `termux-exec` package, clone its repo and build it with a local install prefix, and pass it to `termux-exec` package `make` build.
