...
int execve_intercept(const char *orig_executable_path, char *const argv[], char *const envp[]) {
    ...
    if (executable_path[0] == '/') {
        ...
        executable_path = termux_prefix_path(LOG_TAG, NULL, executable_path,
    ...
    if (inspect_file_header(NULL, header, read_bytes, &info) != 0) {
        return -1;
    }
    ...
    int system_linker_exec = should_system_linker_exec(executable_path);
...
int inspect_file_header(const char *termux_prefix_dir, char *header, size_t header_len,
    ...
    *newline_location = 0;
    ...
    if (whitespace_pos != NULL) {
        ...
        *whitespace_pos = 0;
        ...
        if (interpreter_arg != newline_location) {
            size_t interpreter_arg_buffer_len = sizeof(info->interpreter_arg_buffer);

            size_t interpreter_arg_len = strlen(interpreter_arg);
            if (interpreter_arg_buffer_len <= interpreter_arg_len) {
                #ifndef TERMUX_EXEC__RUNNING_TESTS
                logErrorDebug(LOG_TAG, "The interpreter argument '%s' with length '%zu' is too long to fit in the buffer with length '%zu'",
                    interpreter_arg, interpreter_arg_len, interpreter_arg_buffer_len);
                #endif
                errno = ENAMETOOLONG;
                return -1;
            }

            strcpy(info->interpreter_arg_buffer, interpreter_arg);
    ...
    if (interpreter_path[0] == '/') {
        ...
        info->interpreter_path = termux_prefix_path(LOG_TAG, termux_prefix_dir,
        ...
        strcpy(info->interpreter_path_buffer, interpreter_path);
...
int should_system_linker_exec(const char *executable_path) {
    ...
    if (system_linker_exec_config == 0) { // disable
        system_linker_exec = false;
            ...
            system_linker_exec_available = get_android_build_version_sdk() >= 29;
            logErrorVVerbose(LOG_TAG, "system_linker_exec_available: '%d'", system_linker_exec_available);

            if (system_linker_exec_available) {
                int is_executable_under_termux_app_data_dir = is_path_under_termux_app_data_dir(LOG_TAG,
                    executable_path, NULL, NULL);
                if (is_executable_under_termux_app_data_dir < 0) {
                    return -1;
                }
                logErrorVVerbose(LOG_TAG, "is_executable_under_termux_app_data_dir: '%d'",
                    is_executable_under_termux_app_data_dir == 0 ? true : false);

                system_linker_exec = is_executable_under_termux_app_data_dir == 0;
            }
        ...
        if (get_android_build_version_sdk() >= 29) {
            ...
            if (!app_data_file_exec_exempted) {
                ...
                system_linker_exec = is_executable_under_termux_app_data_dir == 0;
                logErrorVVerbose(LOG_TAG, "is_executable_under_termux_app_data_dir: '%d'", system_linker_exec);
            }
        }
    }
