 ...
 * terminator.
 *
 * If `termux-exec` is set in `LD_PRELOAD` and
...
struct file_header_info {
    ...
    char interpreter_path_buffer[FILE_HEADER__INTERPRETER_PATH___MAX_LEN];
    ...
    char interpreter_arg_buffer[FILE_HEADER__INTERPRETER_ARG___MAX_LEN];
};
...
/**
 * The list of variables that are unset by `modify_exec_env()` if
 * `unset_ld_vars_from_env` is `true`.
 */
 ...
 */
int inspect_file_header(const char *termux_prefix_dir, char *header, size_t header_len,
 ...
 *   are exempted.
 * - `is_path_under_termux_app_data_dir()` returns `true` for the `executable_path`.
 *
 ...
 */
int should_system_linker_exec(const char *executable_path);
