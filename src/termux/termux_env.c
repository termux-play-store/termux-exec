...
int get_android_build_version_sdk() {
    ...
    if (value != NULL && strlen(value) > 0) {
        char build_version_sdk_string[strlen(value) + 1];
        strcpy(build_version_sdk_string, value);
        int build_version_sdk = string_to_int(build_version_sdk_string, -1,
            LOG_TAG, "Failed to convert '%s' env variable value '%s' to an int", ENV__ANDROID__BUILD_VERSION_SDK, build_version_sdk_string);
...
int get_termux_exec__tests_log_level() {
    return get_log_level_value(ENV__TERMUX_EXEC__TESTS__LOG_LEVEL);
}
