...
static char sTermuxLegacyAppDataDirBuffer[TERMUX_APP__DATA_DIR___MAX_LEN];
...
int get_termux_app_data_dir_from_env(const char* log_tag, char *buffer, size_t buffer_len) {
    return get_path_from_env(log_tag, "app_data_dir",
        ENV__TERMUX_APP__DATA_DIR, TERMUX_APP__DATA_DIR___MAX_LEN,
        buffer, buffer_len);
}
...
int get_termux_legacy_app_data_dir_from_env(const char* log_tag, char *buffer, size_t buffer_len) {
    return get_path_from_env(log_tag, "legacy_app_data_dir",
        ENV__TERMUX_APP__LEGACY_DATA_DIR, TERMUX_APP__DATA_DIR___MAX_LEN,
        buffer, buffer_len);
}
...
const char* get_termux_legacy_app_data_dir_from_env_or_default(const char* log_tag, char *buffer, size_t buffer_len) {
    ...
    if (!string_starts_with(termux_legacy_app_data_dir, "/data/data/")) {
        char termux_legacy_app_data_dir_copy[strlen(termux_legacy_app_data_dir) + 1];
        strcpy(termux_legacy_app_data_dir_copy, termux_legacy_app_data_dir);
...
int get_termux_rootfs_dir_from_env(const char* log_tag, char *buffer, size_t buffer_len) {
    return get_path_from_env(log_tag, "rootfs_dir",
        ENV__TERMUX__ROOTFS, TERMUX__ROOTFS_DIR___MAX_LEN,
        buffer, buffer_len);
}
...
int get_termux_prefix_dir_from_env(const char* log_tag, char *buffer, size_t buffer_len) {
    return get_path_from_env(log_tag, "prefix_dir",
        ENV__TERMUX__PREFIX, TERMUX__PREFIX_DIR___MAX_LEN,
        buffer, buffer_len);
}
...
const char* get_termux_tmp_dir(const char* log_tag) {
    if (sTermuxTmpDir == NULL) {
        sTermuxTmpDir = get_termux_tmp_dir_from_env_or_default(log_tag,
...
int get_termux_tmp_dir_from_env(const char* log_tag, char *buffer, size_t buffer_len) {
    return get_path_from_env(log_tag, "tmp_dir",
        ENV__TMPDIR, TERMUX__PREFIX__TMP_DIR___MAX_LEN,
        buffer, buffer_len);
}

const char* get_termux_tmp_dir_from_env_or_default(const char* log_tag, char *buffer, size_t buffer_len) {
    return get_path_from_env_or_default(log_tag, "tmp_dir",
        ENV__TMPDIR, TERMUX__PREFIX__TMP_DIR___MAX_LEN,
...
char *termux_prefix_path(const char* log_tag, const char *termux_prefix_dir, const char *executable_path,
    ...
    char termux_bin_path[TERMUX__PREFIX__BIN_DIR___MAX_LEN + 1];
