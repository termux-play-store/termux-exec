 ...
 * `get_termux_tmp_dir_from_env_or_default()` on its first call.
 */
 ...
 * `-1` on other failures.
 */
int get_termux_tmp_dir_from_env(const char* log_tag,
    char *buffer, size_t buffer_len);
 ...
 * `TERMUX__PREFIX__TMP_DIR` set by `Makefile` if it is readable and executable.
 *
 * @return Returns the `char *` to tmp directory on success,
 * otherwise `NULL`.
 */
const char* get_termux_tmp_dir_from_env_or_default(const char* log_tag,
    char *buffer, size_t buffer_len);
 ...
 *
 * The buffer size must at least be `TERMUX__PREFIX_DIR___MAX_LEN` + strlen(executable_path)`,
