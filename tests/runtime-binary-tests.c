...
static const char* LOG_TAG = "bin";

static uid_t UID;

#define TERMUX_EXEC__TESTS__TESTS_PATH TERMUX__PREFIX "/libexec/installed-tests/termux-exec"
#define TERMUX_EXEC__TESTS__TEST_FILES_BASENAME "files"
#define TERMUX_EXEC__TESTS__EXEC_TEST_FILES_BASENAME "exec"
...
int fork_child(fork_info *info) {
    ...
    if (cpid == 0) {
        ...
        // Close both pipe ends (pipe_fds[0] belongs to parent,
        // pipe_fds[1] no longer needed after dup2).
        close(info->pipe_fds[0]);
        close(info->pipe_fds[1]);
        ...
        if (setvbuf(stdout, NULL, _IONBF, 0) != 0) {
            logStrerror(LOG_TAG, "Child: Failed to set no buffering for stdout");
            exit_fork_with_error(info, 1);
        }
        if (setvbuf(stderr, NULL, _IONBF, 0) != 0) {
            logStrerror(LOG_TAG, "Child: Failed to set no buffering for stderr");
            exit_fork_with_error(info, 1);
        }
...
#define run_exec_test(test_name,                                                                   \
    ...
    variant, name, envp, ...)                                                                      \
    if (1) {                                                                                       \
    logVVVerbose(LOG_TAG, "%s_exec_%s()", test_name, EXEC_VARIANTS_STR[variant]);                  \
                                                                                                   \
    INIT_FORK_INFO(info);                                                                          \
...
#define run_all_exec_wrappers_test(test_name,                                                      \
    ...
    if (1) {                                                                                       \
                                                                                                   \
    logVVerbose(LOG_TAG, "%s_exec()", test_name);                                                  \
                                                                                                   \
    {                                                                                              \
        /* ExecVE */                                                                               \
...
void testExec__Files(const char* termux_exec__tests__tests_path, const char* current_path, char* env_current_path);
void testExec__PackageManager();

void runExecTests() {
    logDebug(LOG_TAG, "runExecTests()");

    const char* termux_exec__tests__tests_path = getenv(ENV__TERMUX_EXEC__TESTS__TESTS_PATH);
    if (termux_exec__tests__tests_path != NULL) {
        size_t termux_exec__tests__tests_path_len = strlen(termux_exec__tests__tests_path);
        if (termux_exec__tests__tests_path_len > 0) {
            if (termux_exec__tests__tests_path[0] != '/' || termux_exec__tests__tests_path_len >= PATH_MAX) {
                logError(LOG_TAG, "Invalid termux_exec__tests__tests_path with length '%zu' set in '%s' env variable: '%s'",
                termux_exec__tests__tests_path_len, ENV__TERMUX_EXEC__TESTS__TESTS_PATH, termux_exec__tests__tests_path);
                logErrorVVerbose(LOG_TAG, "The termux_exec__tests__tests_path must be an absolute path starting with a '/' with max length '%d' including the null '\0' terminator",
                PATH_MAX);
                exit(1);
            }
        } else {
            termux_exec__tests__tests_path = NULL;
        }
    }

    if (termux_exec__tests__tests_path == NULL) {
        termux_exec__tests__tests_path = TERMUX_EXEC__TESTS__TESTS_PATH;
    }
    ...
    testExec__Basic();
    testExec__Files(termux_exec__tests__tests_path, current_path, env_current_path);
...
void testExec__Files(const char* termux_exec__tests__tests_path, const char* current_path, char* env_current_path) {
    ...
    char* termux_exec__exec_test_files_path = NULL;
    asprintf_wrapper(&termux_exec__exec_test_files_path, "%s/%s/%s",
        termux_exec__tests__tests_path, TERMUX_EXEC__TESTS__TEST_FILES_BASENAME, TERMUX_EXEC__TESTS__EXEC_TEST_FILES_BASENAME);
    ...
    asprintf_wrapper(&test_file_path, "%s/%s", termux_exec__exec_test_files_path, "print-args-binary");
    run_all_exec_wrappers_test("print-args-binary",
        0, 0,
        0, "^goodbye-world$", REG_EXTENDED,
        "print-args-binary", test_file_path, environ,
        "goodbye-world", NULL);
    free(test_file_path);

    asprintf_wrapper(&test_file_path, "%s/%s", termux_exec__exec_test_files_path, "print-args-binary.sym");
    run_all_exec_wrappers_test("print-args-binary.sym",
        0, 0,
        0, "^goodbye-world$", REG_EXTENDED,
        "print-args-binary.sym", test_file_path, environ,
        "goodbye-world", NULL);
    free(test_file_path);


    asprintf_wrapper(&test_file_path, "%s/%s", termux_exec__exec_test_files_path, "print-args-linux-script.sh");
    run_all_exec_wrappers_test("print-args-linux-script.sh",
        0, 0,
        0, "^goodbye-world$", REG_EXTENDED,
        "print-args-linux-script.sh", test_file_path, environ,
        "goodbye-world", NULL);
    free(test_file_path);

    asprintf_wrapper(&test_file_path, "%s/%s", termux_exec__exec_test_files_path, "print-args-linux-script.sh.sym");
    run_all_exec_wrappers_test("print-args-linux-script.sh.sym",
        0, 0,
        0, "^goodbye-world$", REG_EXTENDED,
        "print-args-linux-script.sh.sym", test_file_path, environ,
        "goodbye-world", NULL);
    free(test_file_path);


    asprintf_wrapper(&test_file_path, "%s/%s", termux_exec__exec_test_files_path, "print-args-termux-script.sh");
    run_all_exec_wrappers_test("print-args-termux-script.sh",
        0, 0,
        0, "^goodbye-world$", REG_EXTENDED,
        "print-args-termux-script.sh", test_file_path, environ,
        "goodbye-world", NULL);
    free(test_file_path);

    asprintf_wrapper(&test_file_path, "%s/%s", termux_exec__exec_test_files_path, "print-args-termux-script.sh.sym");
    run_all_exec_wrappers_test("print-args-termux-script.sh.sym",
        0, 0,
        0, "^goodbye-world$", REG_EXTENDED,
        "print-args-termux-script.sh.sym", test_file_path, environ,
        "goodbye-world", NULL);
    free(test_file_path);
    ...
    free(termux_exec__exec_test_files_path);
    free(env_new_path);
...
void testExec__PackageManager() {
    ...
    char* termux_package_manager = getenv(ENV__TERMUX_ROOTFS__PACKAGE_MANAGER);
    ...
    asprintf_wrapper(&termux_package_manager_path, "%s/bin/%s", TERMUX__PREFIX, termux_package_manager);
